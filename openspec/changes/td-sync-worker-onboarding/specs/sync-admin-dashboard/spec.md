...
The admin sync panel SHALL render a different resolve form depending on `TdMatchFailure.reason`: `UNMATCHED` offers "link to an existing worker" (as today) or "create a new worker" (name/rate/currency/paymentType/payment account, pre-filled with the TD name/email); `NEEDS_SETUP` offers paymentType + rate/currency/payment account for the already-matched worker; `MISSING_RATE` offers rate/currency only.

#### Scenario: Admin creates a worker from an unmatched failure
- **WHEN** an admin resolves an `UNMATCHED` failure via "create a new worker" and fills in rate/currency/paymentType
- **THEN** a `Worker` is created via the shared creation function, the failure is resolved, and its backfilled invoice is created
